...
if [[ "$SUB" == *"$PREFIX"* ]]; then
    ...
    sed -i "s/DATA_EXTRACTED_TOPIC.*/DATA_EXTRACTED_TOPIC: '$PREFIX.$INBOUND_TOPIC_NAME'/" "$CONFIG_PATH"
...
if [[ "$SUB" == *"$PREFIX"* ]]; then
    ...
    sed -i "s/DATA_PREPARED_TOPIC.*/DATA_PREPARED_TOPIC: '$PREFIX.$OUTBOUND_TOPIC_NAME'/" "$CONFIG_PATH"
...
gcloud functions deploy "$DEPLOYMENT_NAME""_""$SOLUTION_PREFIX""_prepare_daily_transactions" \
   --runtime python37 \
   --entry-point main \
