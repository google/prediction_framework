...
INBOUND_TOPIC_NAME=$PREDICT_TRANSACTIONS_TOPIC
OUTBOUND_TOPIC_NAME=$PREDICT_TRANSACTION_TOPIC
...
if [[ "$SUB" == *"$PREFIX"* ]]; then
    ...
    sed -i "s/PREDICT_TRANSACTIONS_TOPIC.*/PREDICT_TRANSACTIONS_TOPIC: '$PREFIX.$INBOUND_TOPIC_NAME'/" "$CONFIG_PATH"
...
if [[ "$SUB" = *"$PREFIX"* ]]; then
    ...
    sed -i "s/PREDICT_TRANSACTION_TOPIC.*/PREDICT_TRANSACTION_TOPIC: '$PREFIX.$OUTBOUND_TOPIC_NAME'/" "$CONFIG_PATH"
...
gcloud functions deploy "$DEPLOYMENT_NAME""_""$SOLUTION_PREFIX""_predict_transactions" \
   --runtime python37 \
   --entry-point main \
