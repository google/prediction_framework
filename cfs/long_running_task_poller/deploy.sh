...
INBOUND_TOPIC_NAME=$POLLING_LONG_RUNNING_TASKS_TOPIC

SUB=$(cat $CONFIG_PATH | grep -P POLLING_LONG_RUNNING_TASKS_TOPIC)
...
CFG_FILE=$(cat $CONFIG_PATH > ./__config.yaml)

gcloud functions deploy "$DEPLOYMENT_NAME""_""$SOLUTION_PREFIX""_long_running_task_poller" \
   --runtime python37 \
   --entry-point main \
