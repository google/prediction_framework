...
INBOUND_TOPIC_NAME=$STOP_MODEL_TOPIC


SUB=$(cat ../../deploy/config.yaml |  grep -P STOP_MODEL_TOPIC)
...
if [[ "$SUB" == *"$PREFIX"* ]]; then
    ...
    sed -i "s/STOP_MODEL_TOPIC.*/STOP_MODEL_TOPIC: '$PREFIX.$INBOUND_TOPIC_NAME'/" "$CONFIG_PATH"
...
gcloud functions deploy "$DEPLOYMENT_NAME""_""$SOLUTION_PREFIX""_stop_model" \
   --runtime python37 \
   --entry-point main \
