...
INBOUND_TOPIC_NAME="$ENQUEUE_TASK_TOPIC"


SUB=$(cat $CONFIG_PATH |  grep -P ENQUEUE_TASK_TOPIC)
...
if [[ "$SUB" == *"$PREFIX"* ]]; then
    ...
    sed -i "s/ENQUEUE_TASK_TOPIC.*/ENQUEUE_TASK_TOPIC: '$PREFIX.$INBOUND_TOPIC_NAME'/" "$CONFIG_PATH"
